...
function read_parts()
{
    ...
    echo '<table class="table table-bordered table-striped">';
    echo '<thead>';
    echo '<tr><th>Part No.</th><th>Part Name</th><th>Amount</th><th>Lock</th><th>Color</th><th></th></tr>';
    echo '</thead>';
    if (count($parts) > 0) {
        foreach ($parts as $part) {
            ...
            echo '<td style="text-align: center;">' . $part->color . '</td>';
            echo '<td style="text-align: center;">';
...
function save_part($post_data)
{
    ...
    if ($part_id == 0) {
        $query = "INSERT INTO {$tblParts}  (`part_no`, `part_name`, `amount`, `sf`, `ps`, `fl`,`color`)
                    value ('{$part_no}', '{$part_name}', '{$amount}', ${sf}, ${ps}, ${fl}, '${color}')";
    } else {
        $query = "UPDATE {$tblParts} SET `part_no` = '{$part_no}', `part_name` = '{$part_name}', `amount` = '{$amount}', `sf` = '{$sf}', `ps` = '{$ps}', `fl` = '{$fl}', `color` = '{$color}' WHERE `id` = {$part_id}";
    }
    $result = $db->query($query);
    if ($result)
        echo 'Ok';
...
function get_box_data($post_data)
{
    ...
    // if ($area == 'Free Location')
    echo '<div class="col-md-4" style="text-align:center; color: yellow"><p class="h2 text-center">Area: ' . $area . '</p></div>';
    // echo '<div class="col-md-4"></div>';
    // else
    // echo '<td colspan="2">Area: ' . $area . '</td>';
    $colspan = $height - 1;
    // echo '<div class="cold-md-2">Lane: ' . $lane->lane_no . '</div>';
    echo '<div class="col-md-4" style="text-align:center; color: red"><p class="h2">Lane: ' . $lane->barcode_in . '</p></div>';
    // echo '</tr>';
    ...
    foreach ($data as $item) {
        echo '<div class="row">';
        echo '<div class="col-md-4" style="text-align: center;"><p class="h2">Date IN: ' . $item['date'] . '</p></div>';
        echo '<div class="col-md-3" style="text-align: center; color:green;"><p class="h2">Member: ' . $item['member'] . '</p></div>';
        echo '<div class="col-md-2" style="text-align: center;"><p class="h2">Part: ' . $item['part_no'] . '</p></div>';
        echo '<div class="col-md-3" style="text-align: center; color:blue;"><p class="h2">Amount: ' . $item['amount'] . '</p></div>';
        echo '</div>';
        $cols++;
    }
    if ($height > 1)
...
function read_area_lane_status_overview($post_data, $direction = NULL)
{
    ...
    foreach ($areas as $area => $lanes) {
        ...
        foreach ($lanes as $lane) {
            if ($lane['filled_allocation'] > 0 && $lane["filled_allocation"] >= $default_lane_stock && $lane["filled_allocation"] <= $lane["allocation"]) {
                $area_have_most_stock = $area;
            ...
            //     array_push($stock_available_lanes, explode(" ", $lane['lane'])[1]);
            // else
            array_push($stock_available_lanes, explode(" ", $lane['lane_old'])[1]);
        }
